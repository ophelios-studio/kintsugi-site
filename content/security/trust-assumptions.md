...
`@kintsugi/core` is the headless TypeScript library that builds and signs everything. `@kintsugi/cli` and `@kintsugi/ui` are surfaces over it. All three are MIT-licensed, in one public monorepo, with no minified or obfuscated bundles.
