...
Custom calls are a TypeScript API today. They are not exposed in the UI's Discovery panel. Developers can write a small script importing `@kintsugi/core` to compose batches with custom calls; non-developers cannot.
...
Helper functions exist in `@kintsugi/core` (`setResolver`, `clearReverseRecord`) and can be added to the rescue batch via custom calls if you want them executed atomically.
