...
It's also useful in calm: developers building custom rescue flows for non-standard assets (staked positions, vesting, governance) get a teammate who already knows the API and the ordering rules, instead of paging through `@kintsugi/core` source.
